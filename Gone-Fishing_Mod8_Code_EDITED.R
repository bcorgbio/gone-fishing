...
#CAN START RUNNING CODE FROM HERE 
...
options(noaakey = "	vHIJRXdKgYoblxYcaLlvooLzjYxhXNtF")
sts <- c(
...
head(weather.d) #FOCUS: tmin, tmax, awnd, wdf2 

#COMPUTING ARRIVAL TIME 
for (i in species) {mc<- dat%>%
  filter(species==i)%>%
  group_by(year)%>%
...
mc.arrive.date <-mc.pred%>%  #Finding Julianday closest to 25% pop arriving i.e. yearly arrival date of birds to MA
  group_by(year)%>%
...
#COMPUTING PREDICTED MEAN ARRIVAL TIME OVER 2 WEEKS BEFORE EXPECTED ARRIVAL
...
Anova(mc.lmer) #SINGLE weather-day sample

#Mean TWO WEEKS weather preceding arrival
mc.lmer2 <- lmer(j.day~wk.tmin*wk.tmax*wk.wvec+(1|name),mc.arr.weath2,na.action = "na.fail")

Anova(mc.lmer2) 

mc.arr.aic <- dredge(mc.lmer2,fixed = c("wk.tmin","wk.tmax","wk.wvec"),) #removing weather variables

mc.kb <- kable(mc.arr.aic[1:4,],caption = "Fit values for nested models of the most complicated lme model")

kable_styling(mc.kb) #1st result = best model 

#Used to report ANOVA of best model from previous code
